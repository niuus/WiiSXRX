...
//	EventCB[ev][spec].status = SWAP32(EvStALREADY);
	if (EventCB[ev][spec].mode == SWAP32(EvMdINTR)) {
		softCall2(SWAP32(EventCB[ev][spec].fhandler));
	} else EventCB[ev][spec].status = SWAP32(EvStALREADY);
}
...
void psxBios_OpenEvent() { // 08
	...
	EventCB[ev][spec].status = SWAP32(EvStWAIT);
	EventCB[ev][spec].mode = SWAP32(a2);
	EventCB[ev][spec].fhandler = SWAP32(a3);
...
void psxBios_CloseEvent() { // 09
	...
	EventCB[ev][spec].status = SWAP32(EvStUNUSED);
...
#endif
	if (EventCB[ev][spec].status == SWAP32(EvStUNUSED))
	{
		v0 = 0;
		pc0 = ra;
		return;
...
void psxBios_EnableEvent() { // 0c
	...
	EventCB[ev][spec].status = SWAP32(EvStACTIVE);
...
void psxBios_UnDeliverEvent() { // 0x20
	...
	if (EventCB[ev][spec].status == SWAP32(EvStALREADY) &&
		EventCB[ev][spec].mode == SWAP32(EvMdNOINTR))
		EventCB[ev][spec].status = SWAP32(EvStACTIVE);
...
void psxBiosException() {
	...
	switch (psxRegs.CP0.n.Cause & 0x3c) {
		case 0x00: // Interrupt
			interrupt_r26=psxRegs.CP0.n.EPC; // TO DO
#ifdef PSXCPU_LOG
